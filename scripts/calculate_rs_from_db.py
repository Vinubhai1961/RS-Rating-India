...
def log_missing_rs(ticker: str, message: str, log_path: str):
    ...
    call. The old version did open+write+close roughly 10 times per ticker.
    """
...
def calculate_smas(closes: pd.Series):
    ...
    Calculate daily and weekly moving averages:
      - SMA20   (added, matches the NYSE/USA script)
      - SMA50
      - SMA200
      - SMA10W
      - SMA30W

    Original SMA50/SMA200/SMA10W/SMA30W logic is unchanged; SMA20 is added
    using the same rolling-mean + round(2) convention.
    """
    sma20 = np.nan
    sma50 = np.nan
    sma200 = np.nan
    ...
    if len(closes) >= 20:
        sma20 = round(float(closes.rolling(window=20).mean().iloc[-1]), 2)
...
def generate_tradingview_csv(
    ...
    # UTC-consistent: every other date comes from pd.to_datetime(unit="s"),
    # which is UTC, while datetime.fromtimestamp() is LOCAL time.
    latest_date = pd.to_datetime(latest_ts, unit="s").date()
...
def main(
    ...
    # crashed later with a confusing NoneType error instead of this message.
    ...
    for ticker in tqdm(tickers, desc="Calculating RS"):
        ...
        try:
            ...
            if len(closes) < 2:
                ...
                # 15 values, must match the df_stocks column list exactly.
                rs_results.append((
                    ticker,        # Ticker
                    np.nan,        # RS
                    np.nan,        # 1M_RS
                    ...
                    np.nan,        # Last_Close
                    np.nan,        # Prev_Close
                    len(closes)    # History_Days
                ))
                continue

            # === NEW: SMA (now includes SMA20) ===
            sma20, sma50, sma200, sma10w, sma30w = calculate_smas(closes)

            # === NEW: Last_Close / Prev_Close ===
            # closes is already sorted ascending by date, so iloc[-1] is the
            # latest bar and iloc[-2] is the previous session's close.
            ...
            if pd.isna(sma20):
                quality_stats["missing_sma20"] += 1
            if pd.isna(sma50):
                quality_stats["missing_sma50"] += 1
            if pd.isna(prev_close):
                quality_stats["missing_prev_close"] += 1
            if pd.isna(sma200):
    ...
    # ====================== GUARANTEE Prev_Close / SMA20 ======================
    # These are required by downstream consumers. Recover them from merge
    # suffixes if pandas renamed them, otherwise create them as empty columns
    # so rs_stocks.csv always carries the full header.
    for col in ["Prev_Close", "SMA20"]:
        if col not in df_stocks.columns:
            if f"{col}_x" in df_stocks.columns:
                df_stocks = df_stocks.rename(columns={f"{col}_x": col})
            elif f"{col}_y" in df_stocks.columns:
                df_stocks = df_stocks.rename(columns={f"{col}_y": col})
            else:
                df_stocks[col] = np.nan

    df_stocks["Prev_Close"] = pd.to_numeric(
        df_stocks["Prev_Close"], errors="coerce"
    ).round(4)

    logging.info(
        f"Prev_Close missing: {int(df_stocks['Prev_Close'].isna().sum()):,} | "
        f"SMA20 missing: "
        f"{int(pd.to_numeric(df_stocks['SMA20'], errors='coerce').isna().sum()):,}"
    )
    ...
    # Fail loudly instead of silently shipping a CSV without these.
    for required in ["Prev_Close", "SMA20"]:
        if required not in available_cols:
            raise RuntimeError(
                f"Required column {required} missing from rs_stocks output"
            )
    ...
    # order and ticker lists come out ranked without re-scanning the whole
    ...
    missing_sma20_total = int(df_stocks["SMA20"].isna().sum()) if "SMA20" in df_stocks.columns else 0
    missing_sma50_total = int(df_stocks["SMA50"].isna().sum()) if "SMA50" in df_stocks.columns else 0
    missing_prev_close_total = int(df_stocks["Prev_Close"].isna().sum()) if "Prev_Close" in df_stocks.columns else 0
...
Missing SMA20                    : {missing_sma20_total:,}
Missing SMA50                    : {missing_sma50_total:,}
Missing Prev_Close               : {missing_prev_close_total:,}
