...
# ====================== NEW: SMA + ADR CALCULATION (Enhanced) ======================
def calculate_smas_and_adr(data):
    """Calculate SMAs and ADR% - Works with both close-only and OHLC data"""
    closes = pd.Series(data["close"].values, 
                      index=pd.to_datetime(data["datetime"], unit='s')).sort_index()

    sma50 = sma200 = sma10w = sma30w = adr_pct = np.nan

    # SMA Calculations (same as USA script)
    if len(closes) >= 50:
    ...
    try:
        if all(col in data.columns for col in ['high', 'low', 'close']):
            # Full OHLC available
            highs = pd.Series(data["high"].values, index=pd.to_datetime(data["datetime"], unit='s'))
            lows = pd.Series(data["low"].values, index=pd.to_datetime(data["datetime"], unit='s'))
            daily_range_pct = (highs - lows) / closes * 100
            adr_pct = round(daily_range_pct.rolling(window=20).mean().iloc[-1], 2)
        else:
            # Fallback: Close-to-Close daily volatility
            daily_pct = closes.pct_change().abs() * 100
            adr_pct = round(daily_pct.rolling(window=20).mean().iloc[-1], 2)
    except:
        adr_pct = np.nan

    return sma50, sma200, sma10w, sma30w, adr_pct
...
def main(arctic_db_path, reference_ticker, output_dir, log_file, metadata_file=None, percentiles=None, debug=False):
    ...
    if metadata_file and os.path.exists(metadata_file):
        try:
            ...
            metadata_df = pd.DataFrame([r for r in records if r.get("Ticker")])
    ...
    for ticker in tqdm(tickers, desc="Calculating RS"):
        ...
        try:
            ...
            # ====================== NEW ENHANCEMENTS ======================
            sma50, sma200, sma10w, sma30w, adr_pct = calculate_smas_and_adr(data)
            # ============================================================
            ...
            log_missing_rs(ticker, f"FINAL → RS={rs}, 1M={rs_1m}, 3M={rs_3m}, 6M={rs_6m} | "
                                 f"SMA50={sma50}, SMA200={sma200}, ADR={adr_pct}%", missing_rs_log)
            log_missing_rs(ticker, "-" * 60, missing_rs_log)

            rs_results.append((ticker, rs, rs_1m, rs_3m, rs_6m, sma50, sma200, sma10w, sma30w, adr_pct))
            if not np.isnan(rs):
                valid_rs_count += 1
    ...
    # ====================== REST OF PIPELINE (Original + New Columns) ======================
    df_stocks = pd.DataFrame(rs_results, columns=["Ticker", "RS", "1M_RS", "3M_RS", "6M_RS",
                                                  "SMA50", "SMA200", "SMA10W", "SMA30W", "ADR_Pct"])
    ...
    df_stocks["IPO"] = "No"
    df_stocks.loc[df_stocks.get("Type", pd.Series()) == "ETF", ["Sector", "Industry"]] = "ETF"
    ...
    print(f"\nNSE RS CALCULATION COMPLETE! (with SMA + ADR)")
    print(f"Valid RS: {valid_rs_count:,} / {len(df_stocks):,}")
    print(f"Output → {output_dir}/")
    print(f"   • rs_stocks.csv  ← Now includes SMA50, SMA200, SMA10W, SMA30W, ADR_Pct")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate RS Rating for NSE India vs Benchmark")
